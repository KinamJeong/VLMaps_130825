...
def main(config: DictConfig) -> None:
    ...
    for scene_name in config.scene_names:
        ...
        test_scene = os.path.join(config.data_paths.habitat_scene_dir, scene_name + ".glb")
        ...
        levels = scene.levels
        for level in levels:
            print(level.id, level.aabb.center(), level.aabb.size()) #level.aabb.sizes
            print(
                level.id, level.aabb.center()[1] - level.aabb.size()[1] / 2,
                level.aabb.center()[1] + level.aabb.size()[1] / 2
            )
        # for obj in objs:
        ...
        # = np.array([1.5, height_list[np.random.randint(0, len(height_list) - 1)], 4.0])

...
#main.py
#from vlmaps.utils.llm_utils import get_grounded_category_and_location 
import random
...
def main(config: DictConfig) -> None:
    ...
    for scene_i, scene_id in enumerate(scene_ids):
        ...
        for task_id in range(len(object_nav_task.task_dict)):
            ...
            #place, object_category = get_grounded_category_and_location(instruction_from_file)
            ...
            hab_tf = robot.vlmaps_dataloader.to_habitat_tf()
            robot.set_agent_state(hab_tf)
            print(f"[INFO] Task {task_id} random start at row={row}, col={col}, yaw={yaw_deg}")

            #방을 "kitchen" 또는 "bathroom"으로 설정
            place = ["kitchen" , "living room","kitchen"]
            # place = ["living room","bedroom"]
            #object_categories=['television']
            for cat,pl in zip(object_categories,place):  
                print(f"Navigating to category '{cat}' in place '{pl}'")

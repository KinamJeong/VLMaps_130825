...
from vlmaps.utils.index_utils import find_similar_category_id, get_segment_islands_pos, get_dynamic_obstacles_map_3d, get_exact_category_id
from vlmaps.utils.clip_utils import get_lseg_score

import time # 시간 측정

import numpy as np #max_ids 출력
import pandas as pd #max_ids 출력
...
class VLMap(Map):
    ...
    def index_map(self, language_desc: str, with_init_cat: bool = True):
        if with_init_cat and self.scores_mat is not None and self.categories is not None:
            ...
            #find_similar_category_id는 기존 꺼
            cat_id = find_similar_category_id(language_desc, self.categories)
            #그냥 카테고리 id만 찾아주는 함수
            #cat_id = get_exact_category_id(language_desc,self.categories)
            elapsed = time.time() - start_time
            print(f"[TIME] find_similar_category_id('{language_desc}') took {elapsed:.3f} seconds")
            print(f"[category id] : ('{cat_id}')")
            with open("/home/ivl1/00_workspace/evaluation/vlmaps_time_result_log.txt", "a") as f:
                f.write(f"Time : {elapsed:.3f}, Cat category: {language_desc}\n")
    ...
    def total_index_map(self)-> np.ndarray:
        if self.scores_mat is None or self.categories is None:
             raise RuntimeError("scores_mat is not initialized." \
             " Call init_categories() first.")
        
        max_ids = np.argmax(self.scores_mat, axis=1)
         ##max_ids 출력
        #save_path = "output/max_ids.csv"
        #os.makedirs(os.path.dirname(save_path), exist_ok=True)
        #df = pd.DataFrame(max_ids)
        #df.to_csv(save_path, index=False, header=False)
        #print(f"[INFO] max_ids saved to {save_path}")
        ############################################
    ...
    def get_pos(self, name: str) -> Tuple[List[List[int]], List[List[float]], List[np.ndarray], Any]:
        ...
        #픽셀면적 / bbox 면적 통계 계산하기 #######################################################
        areas_true = []
        areas_bbox = []
        for (rmin, rmax, cmin, cmax) in bbox_list:
            # bbox 내 실제 점유 픽셀 수(True 개수)
            patch = foreground[rmin:rmax, cmin:cmax]
            areas_true.append(int(np.count_nonzero(patch)))
            # bbox 사각형 면적(참고용)
            areas_bbox.append(int((rmax - rmin) * (cmax - cmin)))

        n = len(bbox_list)
        mean_true = float(np.mean(areas_true)) if n else 0.0
        median_true = float(np.median(areas_true)) if n else 0.0
        mean_bbox = float(np.mean(areas_bbox)) if n else 0.0
        median_bbox = float(np.median(areas_bbox)) if n else 0.0

        self.last_area_stats[name] = {
            "n": n,
            "areas_true": areas_true,        # 실제 픽셀 면적 리스트
            "areas_bbox": areas_bbox,        # bbox 면적 리스트(참고)
            "mean_true": mean_true,
            "median_true": median_true,
            "mean_bbox": mean_bbox,
            "median_bbox": median_bbox,
        }
        print(f"[STATS] {name}: n={n} | true_mean={mean_true:.2f}, true_median={median_true:.2f} "
              f"| bbox_mean={mean_bbox:.2f}, bbox_median={median_bbox:.2f}")
        #######################################################################################

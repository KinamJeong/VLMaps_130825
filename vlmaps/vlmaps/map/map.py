...
class Map:
    ...
    def generate_cropped_obstacle_map(self, obstacle_map: np.ndarray) -> np.ndarray:
        ...
        self.rmax = np.max(x_indices)+1
        self.cmin = np.min(y_indices)
        self.cmax = np.max(y_indices)
        print(self.rmin, self.rmax, self.cmin, self.cmax)
    ...
    def load_room_labels(self, room_label_path: str):
        self.room_labels = np.array(pd.read_csv(room_label_path, header=None))

    # main.py
    @staticmethod
    def get_majority_room_id(room_labels, row, col, kernel=2):
        H, W = room_labels.shape
        rmin, rmax = max(0, row - kernel), min(H, row + kernel + 1)
        cmin, cmax = max(0, col - kernel), min(W, col + kernel + 1)
        region = room_labels[rmin:rmax, cmin:cmax].flatten()
        counts = Counter(region)
        if 0 in counts:
            del counts[0]  # 0은 무시
        if counts:
            return counts.most_common(1)[0][0]  # 가장 많은 Room ID
        return 0  # 모두 0이면 그대로 0 유지

    def get_nearest_pos_origin(self, curr_pos: List[float], name: str) -> List[float]:
        ...
        if len(centers) == 0:
            return curr_pos
        # 장소 필터링을 거친 객체의 중심점을 출력
        #print(f"[Filtered Centers for {name}]")
        #for idx, center in enumerate(centers):
        #    print(f"  Object {idx}: center = {center}")

        #수정사항 : 가장 가까운 객체가 아닌, 사용자가 지정한 객체로 이동하기
        #selected_id = None

        #if len(centers)==1:
        #    selected_id=0
        #    print(f"[INFO] Only one object found. Automatically selecting object ID: {selected_id}")
        #else:         
        ...
        #if not centers:
        #    print(f"[WARN] No valid centers remain after room filtering. Returning current position.")
        #    return curr_pos

        selected_id = self.select_nearest_obj(centers, bbox_list, curr_pos)
        print(f"[INFO] Selected nearest object ID: {selected_id}, center: {centers[selected_id]}")
        
        return self.nearest_point_on_polygon(curr_pos, contours[selected_id])
    

    def get_nearest_pos(self, curr_pos: List[float], name: str,place:str = None) -> List[float]:
        contours, centers, bbox_list = self.get_pos(name)
        ids_list = self.filter_small_objects(bbox_list, area_thres=10)

        # 수정사항 1 : 전체 객체들의 중심점을 출력해보기
        #print(f"[Filtered Centers for {name}]")
        #for idx, center in enumerate(centers):
        #    print(f"  Object {idx}: center = {center}")
        ##################################
        ...
        if place is not None and hasattr(self, "room_labels"):
            place2label={
            "bathroom": 1,
            "bedroom": 2,
            "corridor": 3,
            ...
            label_id = place2label.get(place.lower())
            if label_id is not None:
                room_filtered_ids = []
                for i, center in enumerate(centers):
                    row, col = int(center[0]), int(center[1])
                    if (
                        0 <= row < self.room_labels.shape[0] and
                        0 <= col < self.room_labels.shape[1] 
                    ):
                        label_value = self.room_labels[row, col]
                        #print(label_value)
                        if label_value == 0:
                            #print("index == 0")
                            label_value = self.get_majority_room_id(self.room_labels, row, col)

                        if label_value == label_id:
                            room_filtered_ids.append(i)

                if room_filtered_ids:
                    centers = [centers[i] for i in room_filtered_ids]
                    contours = [contours[i] for i in room_filtered_ids]
                    bbox_list = [bbox_list[i] for i in room_filtered_ids]
                    print(f"[INFO] Filtered objects in room '{place}' (label={label_id}): {len(centers)}")
                else:
                    print(f"[WARN] No objects found for '{name}' in room '{place}'")
    ...
    def nearest_point_on_polygon(self, coord: List[float], polygon: List[List[float]]):
        ...
        #nearest_coords = self.adjust_to_safe_point(nearest_coords, obs_map=self.get_customized_obstacle_cropped())

        return nearest_coords

    def adjust_to_safe_point(self, point: List[int], obs_map: np.ndarray) -> List[int]:
        x, y = point
        if obs_map[y, x] == 0:
            return [x, y] 
        
        print("[WARN] Point inside obstacle. Adjusting to nearby safe location...")

        offsets = [(-1, 0), (1, 0), (0, -1), (0, 1)]
        for dx, dy in offsets:
             new_x = x + dx
             new_y = y + dy
             if (0 <= new_x < self.obs_map.shape[1]) and (0 <= new_y < self.obs_map.shape[0]):
                 if self.obs_map[new_y, new_x] == 0:
                     print(f"[INFO] Adjusted to safe point: ({new_x}, {new_y})")
                     return [new_x, new_y]
        
        print("[ERROR] Failed to adjust point to safe location.")
        return [x, y]
    ...
    def filter_small_objects(self, bbox_list: List[List[int]], area_thres: int = 80) -> List[int]:
        results_ids = []
    ...
    def get_pos_in_between(
        ...
        #디버깅
        print(f"[DEBUG] Found {len(centers_a)} centers for {obj_a_name}, {len(centers_b)} centers for {obj_b_name}")
        ...
        #전방 필터링 없이 모든 객체 사용
        ids_a_list = list(range(len(centers_a)))
        ids_b_list = list(range(len(centers_b)))
        ...
        #디버깅
        print(f"[DEBUG] After size filtering: {obj_a_name}: {ids_a_list}, {obj_b_name}: {ids_b_list}")
        if not ids_a_list or not ids_b_list:
        ...
        # 두 객체의 중심 디버깅 출력
        print(f"Object A ({obj_a_name}) centers (filtered): {ca}")
        print(f"Object B ({obj_b_name}) centers (filtered): {cb}")
        ...
        masked_middle_to_curr_dist = np.where(
            dist_mat <= max_pair_dist,
            middle_to_curr_dist_mat,
            np.inf
        )

        if np.all(np.isinf(masked_middle_to_curr_dist)):
            print(f"[DEBUG] No valid pair within distance threshold {max_pair_dist}")
            return None
        

        # dist_mat = ca_np @ cb_np.T
        id = np.argmin(masked_middle_to_curr_dist)
        row, col = np.unravel_index(id, masked_middle_to_curr_dist.shape)
        #디버깅
        print(f"[DEBUG] Selected pair: row={row}, col={col}, dist_to_curr={middle_to_curr_dist_mat[row, col]:.2f}")
        dist_to_curr = middle_to_curr_dist_mat[row, col]
        dist = dist_mat[row, col]

        #디버깅 :선택된 중심 좌표 출력
        print(f"Selected center of {obj_a_name}: {ca[row]}")
        print(f"Selected center of {obj_b_name}: {cb[col]}")
        print(f"Middle point between selected: {[(ca[row][0] + cb[col][0]) / 2, (ca[row][1] + cb[col][1]) / 2]}")

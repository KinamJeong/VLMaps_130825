...
class LangRobot:
    ...
    def move_to_origin(self, pos: Tuple[float, float]):
    ...
    def move_to_object(self, name: str,place: str = None):
        move_2_object_start_time = time.time()
        self._set_nav_curr_pose()
        pos = self.map.get_nearest_pos(self.curr_pos_on_map, name,place=place)
        move_2_object_elapsed = time.time() - move_2_object_start_time
        print(f"[TIME] move_2_object took {move_2_object_elapsed:.3f} seconds") 
        return self.move_to(pos)

    def move_to_object_origin(self, name: str):
        self._set_nav_curr_pose()
        pos = self.map.get_nearest_pos_origin(self.curr_pos_on_map, name)
        return self.move_to_origin(pos)

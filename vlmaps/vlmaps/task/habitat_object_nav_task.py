...
class HabitatObjectNavigationTask(HabitatTask):
    ...
    def find_closest_object_from_class(self, class_name: str, pos_hab: np.array):
        ...
        for object in class_objects:
            obj_pos = np.array(object.aabb.center())
            #obj_pos = object.aabb.center    
            #obj_size = object.aabb.size()
            obj_size_np = np.array(object.aabb.size())
            #obj_size_np = np.array([obj_size.x, obj_size.y, obj_size.z])
            dist = get_dist_to_bbox_2d(obj_pos[[0, 2]], obj_size_np[[0, 2]], pos_hab[[0, 2]])
    ...
    def test_step(self, sim: habitat_sim.Simulator, action: str, agent_position: np.array = None, vis: bool = False):
        ...
        if action == "stop":
            ...
            save_path = f"stop_frame_{self.curr_subgoal_id}.png"

...
def extract_direct_object_llm_cot(command: str, location: str):
    ...
    prompt = (
        ...
        # f"Step 3: If it is a synonym, convert it to the most likely canonical object name.\n"
        # f"Step 4: Match your final object to one from this list: {list(object2locations.keys())}.\n"
        f"Command: '{command}'\n"
        f"Location: '{location}'\n"
        f"is desk and table similar?"
...
def infer_location(command: str, top_k: int = 3):
    ...
    prompt = (
        f"Which room is best suited for this command and why?\n"
        f"Command: {command}\n"
